...
<script lang="ts">
	...
	let loadPriceMatrix = (plantId: number) => {
		$ax
			.get("/api/admin/PlantPriceSummary/GetMatrix?plantId=" + plantId)
	...
	let save = () => {
		...
		$ax
			.post(
				"/api/admin/PlantPrice/UpdateAllForPlant?plantId=" + editPlantId,
				pp,
			)
			.then(() => {
...
<Modal isShowModal={true} setModal={cancel}>
	...
	<div class="editor" role="form" onclick={(e) => e.stopPropagation()}>
		...
		<div class="price-item" style="margin-bottom:0.5rem;">
			<div class="description" style="font-weight:bold;font-size:0.9rem;">
				Pot Size
			</div>
			<div
				class="price"
				style="font-weight:bold;font-size:0.9rem;text-align:right;"
			>
				Price
			</div>
			<div class="is-avail" style="font-weight:bold;font-size:0.9rem;">
				Is Available
			</div>
		</div>
			...
			<div class="price-item">
				...
				<div class="price">
					<input
						type="text"
						bind:value={p.price}
						class:error-price={!validatePrice(p.price)}
					/>
				</div>
				<div class="is-avail">
					<input type="checkbox" bind:checked={p.isAvailable} />
				</div>
			</div>
		{/each}
