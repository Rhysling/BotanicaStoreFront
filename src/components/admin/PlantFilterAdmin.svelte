		...
		const f = (p: IPlant) => {
			...
			if (paf.filterText) {
				if (paf.filterType === "genus") {
					passesText = p.genus
						.toLowerCase()
						.startsWith(paf.filterText.toLowerCase());
				} else {
					passesText =
						...
						(p.family || "")
							.toLowerCase()
							.includes(paf.filterText.toLowerCase()) ||
						(p.description || "")
							.toLowerCase()
							.includes(paf.filterText.toLowerCase());
				}
...
<div class="search">
	...
	<input
		type="text"
		class="genus-box"
		bind:value={paf.filterText}
		placeholder="Search {paf.filterType}"
	/>
	<input
		type="text"
		class="flag-box"
	...
	/>
	<div class="sep">
		Listed:<input type="checkbox" bind:checked={paf.isListedOnly} />
	</div>
	<div class="sep">
		NWN:<input type="checkbox" bind:checked={paf.isNwNativeOnly} />
	</div>
	<div class="sep">
