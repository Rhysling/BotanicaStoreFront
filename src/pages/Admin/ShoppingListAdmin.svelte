<script lang="ts">
	...
	let filterList = (inclClosed: boolean) => {
		if (!inclClosed) summaryList = master.filter((a) => !a.isClosed);
		else summaryList = master;
	};

	let viewDetail = (listId: number) => {
		$ax
			.get("/api/admin/ShoppingList/GetItems?listId=" + listId)
			.then((response: AxiosResponse<IvwShoppingListItem[]>) => {
				...
				if (sm) {
					detListId = listId;
					detUser = sm.userFullName
						? `${sm.userFullName} (${sm.email})`
						: sm.email;
				...
				detTotalPretax = detailList.reduce(
	...
	let toggleClosed = () => {
		...
		$ax
			.post(
				"/api/admin/ShoppingList/SetIsClosed?listId=" +
					detListId +
					"&isClosed=" +
					(detIsClosed ? "false" : "true"),
			)
			.then(() => {
...
<div class="summary" style="display:{showSummary ? 'block' : 'none'}">
	...
	{#each summaryList as a (a.wlId)}
		<div class="summary-list" class:is-closed={a.isClosed ? true : undefined}>
			<div class="date">
				{a.lastUpdateDateFormatted}<br />&nbsp;{a.createdDateFormatted}
			</div>
			<div class="user">
				{a.userFullName ? `${a.userFullName} (${a.email})` : a.email}
			</div>
			...
			<div>
				<a
					class="detail-link"
					href="/"
					on:click|preventDefault={() => viewDetail(a.wlId)}>Detail</a
				>
			</div>
		</div>
	{:else}
		<div class="empty">No shopping lists.</div>
	{/each}
</div>

<div class="detail" style="display:{showSummary ? 'none' : 'block'}">
	<div class="close">
		<a href="/" on:click|preventDefault={closeDetail}>Back to Lists</a>
	</div>
	<div class="top">
		...
		<div>
			...
			<a href="/" on:click|preventDefault={toggleClosed}
				>{detIsClosed ? "Open" : "Close"} the List</a
			>
		</div>
	</div>
...
<style lang="scss">
	...
	.is-closed {
		...
		// title {
		// color.scale(c.$main-color, $lightness: 5%, $space: oklch);
		// }
	}
