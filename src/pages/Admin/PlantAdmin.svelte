<script lang="ts">
	...
	const savePlant = (p: IPlant) => {
		...
		$ax
			...
			.then(function (response: AxiosResponse<IPlant>) {
				...
				if (isNew) {
					plants = [p, ...plants].sort(
						(a, b) =>
							(a.genus < b.genus ? -10 : 0) + (a.species < b.species ? -1 : 0),
					);
					filteredList = [p, ...filteredList].sort(
						(a, b) =>
							(a.genus < b.genus ? -10 : 0) + (a.species < b.species ? -1 : 0),
					);
			...
			.catch((e) => {
				editError =
					(e.response?.data?.title || "No title provided.") + " See log.";
				console.error({ e });
			});
	};
	...
	const deletePlant = (plantId: number) => {
		$ax
			.post(`/api/admin/Plants/Delete?plantId=${plantId}`)
			.then(() => {
	...
	let handleUpdatePlantToggle = (e: CustomEvent<PlantToggle>) => {
		...
		if (pt.column) {
			if (pt.column === "isFeatured") {
				plants = plants.map((p) =>
					p.plantId === pt.plantId
						? { ...p, isFeatured: pt.val }
						: { ...p, isFeatured: false },
				);
				pagedList = pagedList.map((p) =>
					p.plantId === pt.plantId
						? { ...p, isFeatured: pt.val }
						: { ...p, isFeatured: false },
				);
			...
			if (pt.column === "isListed") {
				plants = plants.map((p) =>
					p.plantId === pt.plantId ? { ...p, isListed: pt.val } : p,
				);
				pagedList = pagedList.map((p) =>
					p.plantId === pt.plantId ? { ...p, isListed: pt.val } : p,
				);
