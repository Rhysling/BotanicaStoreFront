<script lang="ts">
  ...
  let saveItem = (item: ILink) => {
    ...
    $ax.post("/api/Links/Save", item)
    .then((response: AxiosResponse<ILink>) => {
      let item = response.data;

      master = [
        item,
        ...(master.filter(a => a.linkId !== item.linkId))
  ...
  let filterList = (includeDeleted: boolean) => {
    if (!includeDeleted)
      list = master.filter(a => !a.isDeleted);
    else
      list = master;
  };
...
// *** Init ***
  ...
  let init = () => {
    $ax.get("/api/Links/GetAll?includeDeleted=true")
    .then((response: AxiosResponse<ILink[]>) => {
...
<div class="search">
  <div>
    Incl deleted:
    <input type="checkbox" class="filter-box" bind:checked={includeDeleted} on:change={()=>filterList(includeDeleted)} />
  </div>
  <div class="right">
...
<div>
  ...
  {#each list as a}
    <div class="item" class:is-deleted={a.isDeleted ? true : undefined}>
      ...
      <a class="edit-link" href="/" on:click|preventDefault={() => editItem(a.linkId)}>Edit</a>
    </div>
  {:else}
  <div class="empty">
    No listings.
