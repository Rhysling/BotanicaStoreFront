<script lang="ts">
	...
	const getAudit = async () => {
		try {
      const response = await $ax.get<IPicAuditResult>("/api/admin/Pictures/GetAudit");
	...
	const moveOrphans = async () => {
		...
		try {
      const response = await $ax.post<IPicAuditResult>("/api/admin/Pictures/MoveOrphans");
			auditResult = response.data;
			plants = [];
			editPlantId = 0;
    }
	...
	const loadPlants = async () => {
      ...
      const response = await $ax.put<IPlant[]>("/api/admin/Plants/GetForIds", ids);
	...
	const toggleGalleryForPlant = (plantId: number) => {
		editPlantId = (editPlantId === plantId)
			? 0
			: plantId;
	};
    ...
    .then((response: AxiosResponse<IPlantPicId>) => {
      ...
      if (p) {
        let ppidList: IPlantPicId[] = (<IPlantPicId[]>(JSON.parse(p.pics) || [])).filter(a => a.picId !== ppid.picId);
  ...
  const handleDeletePicture = (e: CustomEvent<IPlantPicId>) => {
    ...
    .then(() => {
			let p = plants.find(a => a.plantId === ppid.plantId);
      if (p) {
        p.pics = JSON.stringify((<IPlantPicId[]>(JSON.parse(p.pics) || [])).filter(a => a.picId !== ppid.picId));
        plants = plants.map(a => (a.plantId === p?.plantId) ? p : a);
      }
    })
    .catch((e: AxiosError) => {
	...
	<div class="section">
		...
		<div class="plants">
			...
			<div class="plant" class:active={editPlantId === p.plantId}>
				<a href="/" on:click|preventDefault={() => toggleGalleryForPlant(p.plantId)}>{p.genus} {p.species}</a>
			</div>
